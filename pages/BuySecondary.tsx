...
import { MapPin, Search, FileText, ArrowRight, Shield, Scale, SearchCheck, FileSearch, ChevronDown } from 'lucide-react';
import { Link } from 'react-router-dom';
import { BLOG_POSTS } from '../services/blogData';

// ... [MOCK_PROPERTIES_SECONDARY, CITIES, DISTRICTS omitted for brevity as they are unchanged] ...
...
const RELEVANT_POSTS = BLOG_POSTS.filter(p => p.id.startsWith('buy-sec-')).slice(0, 3);

const BuySecondary: React.FC = () => {
    // ... [Filter State Logic omitted for brevity] ...
    ...
    return (
        <div className="pt-24 min-h-screen">
             <SEO {...PAGE_CONTENT.BUY_SECONDARY.seo} />

             {/* ... [Hero Section and Filters omitted for brevity - same as previous] ... */}
             <Section className="py-8">
                <div className="mb-8 text-center">
                    ...
                    <p className="text-xl text-gray-500 max-w-2xl mx-auto">{PAGE_CONTENT.BUY_SECONDARY.hero.subtitle}</p>
                </div>
                <GlassCard className="mb-12 sticky top-2 lg:top-24 z-30 p-6 border border-indigo-100 shadow-xl bg-white/90 backdrop-blur-xl">
                    {/* ... [Filter Inputs Code] ... */}
                    <div className="flex flex-col gap-4">
                        <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-2 border-b border-gray-100">
                             {[
                                 ...
                                 <button key={type.id} onClick={() => setSelectedType(type.id as any)} className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all whitespace-nowrap ${selectedType === type.id ? 'bg-black text-white shadow-lg' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{type.label}</button>
                             ))}
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                            <div className="md:col-span-3 relative">
                                <select value={city} onChange={e => { setCity(e.target.value); setDistrict(''); }} className="w-full p-3 bg-gray-50 rounded-xl font-bold text-sm appearance-none outline-none focus:ring-2 focus:ring-indigo-200">{CITIES.map(c => <option key={c} value={c}>{c}</option>)}</select>
                                <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16}/>
                            </div>
                            <div className="md:col-span-3 relative">
                                <select value={district} onChange={e => setDistrict(e.target.value)} className="w-full p-3 bg-gray-50 rounded-xl font-bold text-sm appearance-none outline-none focus:ring-2 focus:ring-indigo-200" disabled={!city}><option value="">Любой район</option>{city && DISTRICTS[city]?.map(d => <option key={d} value={d}>{d}</option>)}</select>
                                <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16}/>
                            </div>
                            <div className="md:col-span-6 relative">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                                <input type="text" placeholder="Улица, метро..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-9 p-3 rounded-xl bg-gray-50 text-sm font-medium outline-none focus:bg-white focus:shadow-inner transition-all"/>
                            </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                            {showRoomSelector && (<div className="md:col-span-4 bg-gray-50 p-1 rounded-xl flex justify-between items-center">{['Студия', '1', '2', '3', '4+'].map(room => (<button key={room} onClick={() => toggleRoom(room)} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${selectedRooms.includes(room) ? 'bg-white shadow-sm text-black' : 'text-gray-400 hover:text-gray-600'}`}>{room}</button>))}</div>)}
                            {showAreaInputs && (<div className={`${showRoomSelector ? 'md:col-span-4' : 'md:col-span-3'} grid grid-cols-2 gap-2`}><input type="number" placeholder="Метраж от" value={areaMin} onChange={e=>setAreaMin(e.target.value)} className="w-full p-3 rounded-xl bg-gray-50 text-sm outline-none focus:bg-white focus:shadow-inner"/><input type="number" placeholder="до м²" value={areaMax} onChange={e=>setAreaMax(e.target.value)} className="w-full p-3 rounded-xl bg-gray-50 text-sm outline-none focus:bg-white focus:shadow-inner"/></div>)}
                            {showPlotInputs && (<div className="md:col-span-3 grid grid-cols-2 gap-2"><input type="number" placeholder="Участок от" value={plotMin} onChange={e=>setPlotMin(e.target.value)} className="w-full p-3 rounded-xl bg-gray-50 text-sm outline-none focus:bg-white focus:shadow-inner"/><input type="number" placeholder="до сот." value={plotMax} onChange={e=>setPlotMax(e.target.value)} className="w-full p-3 rounded-xl bg-gray-50 text-sm outline-none focus:bg-white focus:shadow-inner"/></div>)}
                            <div className={`${showRoomSelector ? 'md:col-span-4' : 'md:col-span-3'} grid grid-cols-2 gap-2`}><input type="number" placeholder="Цена от" value={priceMin} onChange={e=>setPriceMin(e.target.value)} className="w-full p-3 rounded-xl bg-gray-50 text-sm outline-none focus:bg-white focus:shadow-inner"/><input type="number" placeholder="до млн ₽" value={priceMax} onChange={e=>setPriceMax(e.target.value)} className="w-full p-3 rounded-xl bg-gray-50 text-sm outline-none focus:bg-white focus:shadow-inner"/></div>
                        </div>
                    </div>
                </GlassCard>
             ...
             <Section title="Почему выбирают вторичку?">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                        { icon: <Shield size={24} />, title: "Юридическая чистота", desc: "Мы проверяем каждый объект по 15 базам перед внесением аванса.", color: "bg-indigo-100 text-indigo-600" },
                        { icon: <MapPin size={24} />, title: "Инфраструктура", desc: "Зеленые дворы, школы и сады с историей — всё уже работает.", color: "bg-purple-100 text-purple-600" },
                        { icon: <FileText size={24} />, title: "Без сюрпризов", desc: "Вы видите, что покупаете: виды из окон, соседей и состояние подъезда.", color: "bg-blue-100 text-blue-600" }
                    ].map((item, i) => (
                        <div key={i} className="apple-panel p-6 cursor-pointer hover:scale-[1.02] transition-transform duration-300 flex flex-col justify-between group shadow-sm hover:shadow-md">
                            <div>
             ...
             <Section title="Наши инструменты">
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                     {[
                         { id: 'tool-history', icon: <SearchCheck/>, title: "Проверка собственника", desc: "Поиск по базам банкротств, ФССП и судов." },
                         { id: 'tool-history', icon: <FileSearch/>, title: "Анализ истории", desc: "Архивные выписки ЕГРН и переходы права." },
                         { id: 'tool-valuation', icon: <Scale/>, title: "Оценка цены", desc: "Сравнительный анализ аналогов для торга." }
                     ].map((tool, i) => (
                         <Link key={i} to={`/blog/${tool.id}`} className="block h-full group cursor-pointer">
                             <GlassCard className="h-full flex flex-col p-6 hover:scale-[1.02] transition-transform duration-300 border border-indigo-100 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)]">
                                 <div className="flex justify-between items-start mb-6">
                                     <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-indigo-100 text-indigo-600 group-hover:bg-black group-hover:text-white transition-colors shadow-sm">
                                         {React.cloneElement(tool.icon as any, { size: 24 })}
                                     </div>
                                 </div>
                                 <h3 className="font-bold text-lg mb-2 group-hover:text-indigo-600 transition-colors">{tool.title}</h3>
             ...
             <Section title="Пакеты услуг">
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                     {[
                         { title: "Сопровождение", price: "100 000 ₽", feats: ["Юр. проверка", "Договор", "Сделка"] },
                         { title: "Под ключ", price: "2.5%", feats: ["Поиск", "Торг", "Проверка", "Сделка", "Приемка"], popular: true },
                         { title: "Юрист", price: "50 000 ₽", feats: ["Анализ документов выбранной квартиры"] }
                     ].map((pkg, i) => (
             ...
             <ExpertsSection title="Эксперты вторичного рынка" colorTheme="indigo" />
             ...
             <ReviewsSection reviews={PAGE_CONTENT.BUY_SECONDARY.reviews} />
             ...
             <Section className="pb-32">
                 <LeadForm title="Нужна помощь с проверкой?" subtitle="Наш юрист проверит историю квартиры и собственников за 24 часа." buttonText="Проверить объект" />
             </Section>
        </div>
    );
};
